...
public class DatabaseNamingStrategy extends DefaultComponentSafeNamingStrategy {
	...
	public String classToTableName(String className) {
		String result ;
		if (isAddUnderscores) {
			result = StringUtils.substring(tablePrefix + super.classToTableName(addUnderscores(className)), 0, maxLength);
		} else {
			result = StringUtils.substring(tablePrefix + super.classToTableName(className), 0, maxLength);
		}
		String regex = "(\\d++)";
        Matcher m = Pattern.compile( regex ).matcher( result );
        while ( m.find() ) {
            result = result.replaceAll(regex, "_"+m.group( 1 )+"_");
        }
		return result;
	}
	...
	public static void main(String[] args) {
		DatabaseNamingStrategy name = new DatabaseNamingStrategy();
		name.setIsAddUnderscores(true);
		name.setTablePrefix("");
		name.setMaxLength(64);
		String result = name.classToTableName("Abc2Efg");
