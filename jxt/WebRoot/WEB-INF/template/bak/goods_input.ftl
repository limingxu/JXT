...
$().ready(function() {
	...
	var $goodsPriceTable = $("#goodsPriceTable");
	var $addGoodsPriceButton = $("#addGoodsPriceButton");
	...
	// 增加价格区间
	var goodsPriceIndex =<#if !(goods??) || !(goods.priceSet??) || (goods.priceSet?size) == 0> 1 <#else> ${(goods.priceSet?size)} </#if>;
	$addGoodsPriceButton.click( function() {
		
		<@compress single_line = true>
			var goodsPriceTrHtml = 
			'<tr class="goodsPriceTr">
				<td>
					<input type="text" name="priceList[' + goodsPriceIndex + '].minnum" class="formText priceMinnum" />
				</td>
				<td>
					<input type="text" name="priceList[' + goodsPriceIndex + '].price" class="formText pricePrice" style="width: 50px;" />
				</td>
				<td>
					<span class="deleteIcon deleteGoodsPrice" title="删 除">&nbsp;</span>
				</td>
			</tr>';
		</@compress>
		
		$goodsPriceTable.append(goodsPriceTrHtml);
		goodsPriceIndex ++;
	});
	...
	//删除价格区间
	$("#goodsPriceTable .deleteGoodsPrice").live("click", function() {
		var $this = $(this);
		$.dialog({type: "warn", content: "您确定要删除吗?", ok: "确 定", cancel: "取 消", modal: true, okCallback: deleteGoodsPrice});
		function deleteGoodsPrice() {
			$this.parent().parent().remove();
		}
	});
	...
	$.validator.addMethod("pricePriceMin", $.validator.methods.min, "价格区间页签下的价格必须为零或正数");
	$.validator.addMethod("priceMinnumMin", $.validator.methods.min, "价格区间页签下的起订数量必须为大于1的正整数");
	$.validator.addMethod("pricePriceRequired", $.validator.methods.required, "请填写价格区间页签下的价格");
	$.validator.addMethod("priceMinnumRequired", $.validator.methods.required, "请填写价格区间页签下的起订数量");
	...
	$.validator.addClassRules("pricePrice", {
		pricePriceRequired: true,
		pricePriceMin: 0
	});
	$.validator.addClassRules("priceMinnum", {
		priceMinnumRequired: true,
		priceMinnumMin: 2
	});
	...
	<div class="body">
		<form id="goodsForm" action="<#if isAddAction>goods!save.action<#else>goods!update.action</#if>" method="post" enctype="multipart/form-data">
			...
			<ul id="tab" class="tab">
				...
				<li>
					<input type="button" value="价格区间" hidefocus />
				</li>
				<li>
			...
			<table id="infoTable" class="inputTable tabContent">
				...
				<tr>
					...
					<td>
						<select name="goods.shop.id">
							<option value="">请选择...</option>
							<#list shops as shop>
								<option value="${shop.id}" <#if (shop.id == goods.shop.id)!> selected</#if>>
									${shop.name}
								</option>
							</#list>
						</select>
			...
			<table id="goodsImageTable" class="inputTable tabContent">
				...
				<table id="goodsPriceTable" class="inputTable tabContent">
				<tr class="noneHover">
					<td colspan="3">
						<input type="button" id="addGoodsPriceButton" class="formButton" value="增加区间" hidefocus />
					</td>
				</tr>
				<tr class="title">
					<th>
				...
				<#if (goods.priceSet)?? && (goods.priceSet?size)!=0>
				<#list (goods.priceSet)! as price>
					<tr class="goodsPriceTr">
						<input type="hidden" name="priceList[${price_index}].id" value="${price.id}" />
						<td>
							<input type="text" name="priceList[${price_index}].minnum" class="formText priceMinnum" value="${price.minnum}" />
						</td>
						<td>
							<input type="text" name="priceList[${price_index}].price" class="formText pricePrice" value="${price.price}" style="width: 50px;" />
						</td>
						<td>
							<span <#if price_index!=0> class="deleteIcon deleteGoodsPrice"</#if> title="删 除">&nbsp;</span>
						</td>
					</tr>
				</#list>
				<#else>
					<tr class="goodsPriceTr">
					<td>
						<input type="text" name="priceList[0].minnum" class="formText priceMinnum" value="" />
					</td>
					<td>
						<input type="text" name="priceList[0].price" class="formText pricePrice" value="" style="width: 50px;" />
					</td>
					<td>
						<span title="删 除">&nbsp;</span>
					</td>
