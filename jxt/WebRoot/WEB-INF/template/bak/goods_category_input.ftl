		...
		<form id="validateForm" action="<#if isAddAction>goods_category!save.action<#else>goods_category!update.action</#if>" method="post">
			...
			<table class="inputTable">
				...
				<tr>
					...
					<td>
						<select name="goodsCategory.busiArea.id">
							<#list subBusiAreas as sub>
								<option value="${sub.id}"<#if (goodsCategory.busiArea.id == sub.id)!> selected</#if>>
				...
				<tr>
					...
					<td>
						<@checkbox name="goodsCategory.isRecommand" value="${(goods.isBest)!false}" />推荐
					</td>
				</tr>
				<tr>
					<th>
