	...
	/**
	 @Validations(
		requiredStrings = {
			...
			@RequiredStringValidator(fieldName = "shop.name", key = I18nKeys.ShopKeys.v_shop_name_required),
			...
			@RequiredStringValidator(fieldName = "shop.qq", message = "QQ号码不允许为�?")
		},
		stringLengthFields = {
			@StringLengthFieldValidator(fieldName = "admin.username", minLength = "2", maxLength = "20", message = "用户名长度必须在${minLength}�?{maxLength}之间!"),
			@StringLengthFieldValidator(fieldName = "shop.name",minLength="1",maxLength="50",key = I18nKeys.ShopKeys.v_shop_name_length),
			@StringLengthFieldValidator(fieldName = "admin.password", minLength = "4", maxLength = "20", message = "密码长度必须�?{minLength}�?{maxLength}之间!"),
			@StringLengthFieldValidator(fieldName = "admin.email", minLength = "1", maxLength = "50", message = "邮箱长度不能超过${maxLength}!"),
			@StringLengthFieldValidator(fieldName = "shop.qq",minLength="1",maxLength="20",key = I18nKeys.ShopKeys.v_shop_qq_length)
		},
		emails = {
		...
		regexFields = {
			...
			@RegexFieldValidator(fieldName = "shop.qq", expression = "^[0-9]+$", message = "店铺名只允许为数�?") 
		}
	)
	@InputConfig(resultName = "error")
	public String saveshoper() {
		if (!CaptchaUtil.validateCaptchaByRequest(getRequest())) {
			addActionError("验证码输入错�?");
			return ERROR;
		}
		...
		if (shopService.isExistsSameNameExceptId(shop.getName(), id)) {
			addActionError(getText(I18nKeys.ShopKeys.v_shop_name_exists));
			return ERROR;
		}
		
		adminService.saveShoper(admin,shop);
		
		logInfo = "添加管理�? " + admin.getUsername();
		redirectUrl = "page!main.action";
		return SUCCESS;
	}*/
